    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        <div>
          ...
          <p className="text-[#a89c93] text-sm">Acompanhe suas receitas, gerencie assinantes e permissões do sistema.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsAddUserModalOpen(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[var(--theme-primary)] hover:bg-[var(--theme-primary-hover)] text-black font-bold text-xs shadow-md transition-all cursor-pointer"
          >
      ...
      {/* Modal Add User / Subscriber */}
      {isAddUserModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#1a1614] border border-[#3d342f] rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            <button
              onClick={() => setIsAddUserModalOpen(false)}
              className="absolute top-4 right-4 text-[#a89c93] hover:text-[#fcf8f5] p-1 rounded-lg hover:bg-[#241e1b]"
            >
              <X className="w-5 h-5" />
            </button>

            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-[var(--theme-badge-bg)] border border-[var(--theme-badge-border)] flex items-center justify-center text-[var(--theme-primary)]">
                <User className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-lg font-serif font-bold text-[#fcf8f5]">Cadastrar Novo Assinante</h3>
                <p className="text-xs text-[#a89c93]">Adicione um novo cliente ou administrador manualmente.</p>
              </div>
            </div>

            <form onSubmit={handleCreateUser} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-[#a89c93] uppercase mb-1">Email do Usuário *</label>
                <input
                  type="email"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-[#12100e] border border-[#3d342f] text-sm text-[#fcf8f5] placeholder-[#8c827a] focus:outline-none focus:border-[var(--theme-primary)]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[#a89c93] uppercase mb-1">Função</label>
                  <select
                    value={newRole}
                    onChange={(e) => setNewRole(e.target.value as 'user' | 'admin')}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-[#12100e] border border-[#3d342f] text-sm text-[#fcf8f5] focus:outline-none focus:border-[var(--theme-primary)]"
                  >
                    <option value="user">Cliente / Assinante</option>
                    <option value="admin">Administrador</option>
                  </select>
                ...
                <div>
                  <label className="block text-xs font-bold text-[#a89c93] uppercase mb-1">Status Inicial</label>
                  <select
                    value={newStatus}
                    onChange={(e) => setNewStatus(e.target.value as 'active' | 'pending' | 'inactive')}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-[#12100e] border border-[#3d342f] text-sm text-[#fcf8f5] focus:outline-none focus:border-[var(--theme-primary)]"
                  >
              ...
              {newRole === 'user' && (
                <div>
                  <label className="block text-xs font-bold text-[#a89c93] uppercase mb-1">Data de Vencimento Inicial</label>
                  <input
                    type="date"
                    value={newDueDate}
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#3d342f]">
                <button
                  type="button"
                  onClick={() => setIsAddUserModalOpen(false)}
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-[#a89c93] hover:text-[#fcf8f5] hover:bg-[#241e1b]"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-[var(--theme-primary)] hover:bg-[var(--theme-primary-hover)] text-black font-bold text-xs flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
